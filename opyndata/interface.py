...
class AppSetup:
    def __init__(self, data_path,
        ...
        self.stats_path = 'stats.mat'    # remove, statistics embedded in h5
    ...
    def create_app(self):
        ...
        sensors_stat = list(statistics['sensor'].keys())
        files = statistics['recording']
        files = [file.split('.mat')[0] for file in files]
        ...
        def update_figure(selected_sensor, selected_component, selected_file, detrend_state, domain, nfft, zp):
            selected_hf = self.hf[selected_file]
            sensor_dict = create_sensor_dict(selected_hf)
           
            try:    
                y = selected_hf[sensor_dict[selected_sensor]][selected_sensor][selected_component]
                x = time_axis(selected_hf, selected_sensor, sensor_dict=sensor_dict)
